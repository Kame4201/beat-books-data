...
class TestBatchScrapeService:
    ...
    async def test_create_batch_job_success(self, batch_service, mock_job_repo):
        ...
        targets = [{"team": "chiefs", "year": 2024}, {"team": "bills", "year": 2024}]

        # Mock job creation
        mock_job = ScrapeJob(
            id=1, status="pending", total_urls=2, processed=0, failed=0
        )
    ...
    async def test_execute_batch_job_success(self, batch_service, mock_job_repo):
        ...
        # Mock job retrieval
        mock_job = ScrapeJob(
            id=job_id, status="pending", total_urls=2, processed=0, failed=0
        )
        mock_job_repo.get_by_id.return_value = mock_job

        # Mock scrape_and_store to avoid actual scraping
        with patch(
            "src.services.batch_scrape_service.scrape_and_store", new_callable=AsyncMock
        ) as mock_scrape:
    ...
    async def test_execute_batch_job_partial_failure(
        self, batch_service, mock_job_repo
    ):
        ...
        # Mock job retrieval
        mock_job = ScrapeJob(
            id=job_id, status="pending", total_urls=3, processed=0, failed=0
        )
        mock_job_repo.get_by_id.return_value = mock_job
        ...
        with patch(
            "src.services.batch_scrape_service.scrape_and_store",
            side_effect=mock_scrape_fn,
        ):
            with patch("asyncio.sleep", new_callable=AsyncMock):
                result = await batch_service.execute_batch_job(job_id, targets)
