...
class GameWeather(Base):
    ...
    weather_condition = Column(String(100), nullable=True)  # clear, cloudy, rain, snow, etc.

    # Metadata
    game_time = Column(DateTime, nullable=True)  # Scheduled game time
    fetched_at = Column(DateTime, nullable=False, default=datetime.now)  # When weather was fetched
