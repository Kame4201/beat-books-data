...
class GameWeatherCreate(BaseModel):
    ...
    week: int = Field(..., ge=0, le=22, description="Week number (0-22, includes playoffs)")
    home_team: str = Field(..., min_length=2, max_length=10, description="Home team abbreviation")
    ...
    precipitation: Optional[float] = Field(None, ge=0, description="Precipitation in inches")
    humidity: Optional[float] = Field(None, ge=0, le=100, description="Humidity percentage")
    weather_condition: Optional[str] = Field(None, max_length=100, description="Weather description")
    game_time: Optional[datetime] = Field(None, description="Scheduled game time")
    fetched_at: Optional[datetime] = Field(default_factory=datetime.now, description="When weather was fetched")
